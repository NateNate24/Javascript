...
function myPages_Added(pg) {
    if (pg.Properties.ID().indexOf("pg1") !== -1) {
        ...
        Vitro.Elements.RegisterRepeaterControls(pg, ["rptGrid", "pnlGrid", "rptSTK", "dpDate", "dpTime", "ddFeelTired", "ddSleeping", "ddMood",
            "ddEasilyAnnoyed", "ddAnxious", "ddAchesPains", "ddAppetite", "ddUnusualThings", "ddSuspicious", "ddConcentration",
            "txtTotal", "txtTemperature", "txtPulse", "txtRespirations", "txtBPSys", "txtBPDias", "ddPupilSizeL", "ddPupilSizeR",
            "txtMedication", "txtAmount", "autSign", "btnStk", "txtStk", "txtInitials", "txtBG"], REPEATER_LENGTH);                 
        // set initial readonly columns
        for (var i = 0; i < REPEATER_LENGTH; i++) {
            pg.rptGrid[i].ReadOnly();
             // set dropdown to center
            pg.Controls("ddFeelTired[" + i + "]", "ddSleeping[" + i + "]", "ddMood[" + i + "]", "ddEasilyAnnoyed[" + i + "]", "ddAnxious[" + i + "]", "ddAchesPains[" + i + "]", "ddAppetite[" + i + "]", "ddUnusualThings[" + i + "]", "ddSuspicious[" + i + "]", "ddConcentration[" + i + "]", "ddPupilSizeL[" + i + "]", "ddPupilSizeR[" + i + "]").Each.Attribute(Vitro.DROPDOWN.TextAlignment, "Center", true);
        }
...
function SetEvents(page) {
    var ddColumn_Controls = page.Controls("ddFeelTired[" + index + "]", "ddSleeping[" + index + "]", 
                            "ddMood[" + index + "]", "ddEasilyAnnoyed[" + index + "]", "ddAnxious[" + index + "]", 
                            "ddAchesPains[" + index + "]", "ddAppetite[" + index + "]", "ddUnusualThings[" + index + "]", 
                            "ddSuspicious[" + index + "]", "ddConcentration[" + index + "]");
...
function DateTimeLastUsed_Change(pg, ctrl, oldValue, newValue) {
    var newDateTimeVal = Vitro.Elements.CombinedDateAndTime(pg.dpDateLastUsed.Value(), pg.dpTimeLastUsed.Value());
    if(ctrl.Value()){
        if(ctrl.Properties.ID() === "dpDateLastUsed"){
            Vitro.Elements.DateTimeValidate(ctrl, newValue, COLOURS.TRANSPARENT, null, null, pg.dpTimeLastUsed);
            var controlDate = new Date(Vitro.Elements.GetDateString(newValue).ddMMyyyy);
            var currentdate = new Date(Vitro.Elements.GetDateString(new Date()).ddMMyyyy);
            if (isNaN(controlDate.getTime()) || controlDate.getTime() > currentdate.getTime()) {
                pg.dpTimeLastUsed.Attribute(Vitro.CONTROL.BackgroundColour, COLOURS.TRANSPARENT, true);
            }
        }
        else{
            Vitro.Elements.DateTimeValidate(ctrl, newDateTimeVal, COLOURS.TRANSPARENT, pg.dpDateLastUsed, pg.dpTimeLastUsed);
        }
    }
    else{
        ctrl.Value(new Date());
...
function Total_Compute(page, control, oldValue, newValue) {
    ...
    SetTotalBGColor(page.txtBG[index], total);
...
function SetTotalBGColor(ctrl, total) {
    // IF TOTAL ≥ 0 and ≤ 10
    if (parseInt(total) >= 0 && parseInt(total) <= 10) {
...
function ClearPopulatedDatetime(page){
    var requiredControls = page.Controls("ddFeelTired[" + index + "]", "ddSleeping[" + index + "]", 
        "ddMood[" + index + "]", "ddEasilyAnnoyed[" + index + "]", "ddAnxious[" + index + "]", 
        "ddAchesPains[" + index + "]", "ddAppetite[" + index + "]", "ddUnusualThings[" + index + "]", 
        "ddSuspicious[" + index + "]", "ddConcentration[" + index + "]");
    var columnCtrls = page.Controls("ddFeelTired[" + index + "]", "ddSleeping[" + index + "]", 
        "ddMood[" + index + "]", "ddEasilyAnnoyed[" + index + "]", "ddAnxious[" + index + "]", 
        "ddAchesPains[" + index + "]", "ddAppetite[" + index + "]", "ddUnusualThings[" + index + "]", 
        "ddSuspicious[" + index + "]", "ddConcentration[" + index + "]", "txtTemperature[" + index + "]",
        "txtPulse[" + index + "]", "txtRespirations[" + index + "]", "txtBPSys[" + index + "]", 
        "txtBPDias[" + index + "]", "ddPupilSizeL[" + index + "]", "ddPupilSizeR[" + index + "]", 
        "txtMedication[" + index + "]", "txtAmount[" + index + "]");
    var isValid = true;
