...
COLOURS.GREEN = "255, 0, 176, 80";
COLOURS.SOLID_DARK_YELLOW = "255, 255, 192, 0";
COLOURS.RED = "255, 255, 0, 0";
COLOURS.TRANSPARENT = "0, 255, 255, 255";
...
var YELLOW = "yellow";
var PINK = "pink";
var SOLID_PINK = "255, 253, 127, 115";
var SOLID_YELLOW = "255, 255, 250, 200";
        ...
        if (page.Properties.ID().indexOf("pg1") !== -1) {
            ...
            for (var j = 0; j < REPEATER_LENGTH; j++) {
                ...
                if (!page.autSign[j].Value() && !index_flag) {
                    ...
                    page.Title("Obs " + Vitro.Elements.GetDateString().ddMM);
                    ShowObsCtrl(pgObsNext, j, true);
                    index_flag = true;     
                    index = j;                   
                    SetEvents(page);                       
            ...
            Vitro.Elements.GetLatestObsEntry(myApp, pgObsNext.txtTemperature[index], pgObsNext.txtPulse[index], pgObsNext.txtRespirations[index], 
                                                pgObsNext.txtBPSys[index], pgObsNext.txtBPDias[index]);
        }        
    }
}
...
function myPages_Added(pg) {
    if (pg.Properties.ID().indexOf("pg1") !== -1) {
        pg.RegisterControl("dpDateLastUsed");
        Vitro.Elements.SetControlHighlight(pg.dpDateLastUsed, null, COLOURS_COPY);
        Vitro.Elements.RegisterRepeaterControls(pg, ["rptGrid", "pnlGrid", "rptSTK", "dpDate", "dpTime", "ddIrritable", "ddFatigued", 
            "ddConcentrating", "ddAppetite", "ddNumbness", "ddHeartRacing", "ddAchy", "ddMuscles", "ddAnxious", "ddUpset", "ddEyesBlurred", 
            "ddFearful", "ddMisfortune", "ddPerspiration", "ddTremor", "ddAgitation", "txtTotal", "txtTemperature", "txtPulse", "txtRespirations", 
            "txtBPSys", "txtBPDias", "ddPupilSizeL", "ddPupilSizeR", "txtMedication", "txtAmount", "autSign", "btnStk", "txtStk", "txtInitials"], REPEATER_LENGTH);                 
          // set initial readonly columns
          for (var i = 0; i < REPEATER_LENGTH; i++) {
                pg.rptGrid[i].ReadOnly();
                // set dropdown to center
               pg.Controls("ddIrritable[" + i + "]", "ddFatigued[" + i + "]", "ddConcentrating[" + i + "]", "ddAppetite[" + i + "]", "ddNumbness[" + i + "]", "ddHeartRacing[" + i + "]", "ddAchy[" + i + "]", "ddMuscles[" + i + "]", "ddAnxious[" + i + "]", "ddUpset[" + i + "]", "ddEyesBlurred[" + i + "]", "ddFearful[" + i + "]", "ddMisfortune[" + i + "]", "ddPerspiration[" + i + "]", "ddTremor[" + i + "]", "ddAgitation[" + i + "]").Each.Attribute(Vitro.DROPDOWN.TextAlignment, "Center", true);
          }
...
function SetEvents(page) {
    var ddColumn_Controls = page.Controls("ddIrritable[" + index + "]", "ddFatigued[" + index + "]", 
                            "ddConcentrating[" + index + "]", "ddAppetite[" + index + "]", "ddNumbness[" + index + "]", 
                            "ddHeartRacing[" + index + "]", "ddAchy[" + index + "]", "ddMuscles[" + index + "]", 
                            "ddAnxious[" + index + "]", "ddUpset[" + index + "]", "ddEyesBlurred[" + index + "]", "ddFearful[" + index + "]", 
                            "ddMisfortune[" + index + "]", "ddPerspiration[" + index + "]", "ddTremor[" + index + "]", "ddAgitation[" + index + "]");
    ...
    page.txtMedication[index].Events.Click(Medication_Click);
...
function DateTimeLastUsed_Change(pg, ctrl, oldValue, newValue) {
    if (ctrl.Value()){
        ...
        ctrl.Value(new Date());
        Vitro.Elements.ChangeBG(ctrl, COLOURS.TRANSPARENT, false);
    }
}
...
function Medication_Click(page, control) {
    var YES = "Yes";
    var NO = "No";
    if (control.Value() === "") {
        control.Value(YES);
...
function SetTotalBGColor(ctrl, total) {
    // IF TOTAL ≥ 0 and ≤ 20
    if (parseInt(total) >= 0 && parseInt(total) <= 20) {
        // Set TOTAL textbox background to GREEN 
        ctrl.Attribute(Vitro.CONTROL.BackgroundColour, COLOURS.GREEN, true);
    } 
    else if (parseInt(total) >= 21 && parseInt(total) <= 40) {
       // Set TOTAL textbox background to YELLOW 
       ctrl.Attribute(Vitro.CONTROL.BackgroundColour, COLOURS.SOLID_DARK_YELLOW, true); 
    }
    else if (parseInt(total) >= 41) {
...
function Authorisation_Change(page, control, oldValue, newValue) {
    ...
    if (newValue) {
        ...
        page.rptGrid[index].Properties.Children().Iterate(function (c) { 
            if (c.Properties.ID().indexOf("autSign") === -1 && c.Properties.ID().indexOf("txtTotal") === -1) {
                if (c.Properties.ID().indexOf("txtMedication") !== -1) { c.Disable(true); }
        ...
        page.rptGrid[index].Properties.Children().Iterate(function (c) { 
            if (c.Properties.ID().indexOf("autSign") === -1 && c.Properties.ID().indexOf("txtTotal") === -1) { 
                if (c.Properties.ID().indexOf("txtMedication") !== -1) { c.Enable(true); }
                else { c.Writeable(true); }
            }
        });
    }
}
...
function CloneObsPage(page) {
    ...
    if (pgObsNext) {        
        ...
        pgObsNext.Order(page.Order()).Deletable(false).Show(true).Title("Obs " + Vitro.Elements.GetDateString().ddMM);        
        myPages_Added(pgObsNext);
        index = 0;
        pgObsNext.rptGrid[index].Writeable();
...
function ClearPopulatedDatetime(page){
    var requiredControls = page.Controls("ddIrritable[" + index + "]", "ddFatigued[" + index + "]", 
        "ddConcentrating[" + index + "]", "ddAppetite[" + index + "]", "ddNumbness[" + index + "]", 
        "ddHeartRacing[" + index + "]", "ddAchy[" + index + "]", "ddMuscles[" + index + "]", 
        "ddAnxious[" + index + "]", "ddUpset[" + index + "]", "ddEyesBlurred[" + index + "]", "ddFearful[" + index + "]", 
        "ddMisfortune[" + index + "]", "ddPerspiration[" + index + "]", "ddTremor[" + index + "]", "ddAgitation[" + index + "]");

    var columnCtrls = page.Controls("ddIrritable[" + index + "]", "ddFatigued[" + index + "]", "ddConcentrating[" + index + "]", 
    "ddAppetite[" + index + "]", "ddNumbness[" + index + "]", "ddHeartRacing[" + index + "]", "ddAchy[" + index + "]", 
    "ddMuscles[" + index + "]", "ddAnxious[" + index + "]", "ddUpset[" + index + "]", "ddEyesBlurred[" + index + "]", 
    "ddFearful[" + index + "]", "ddMisfortune[" + index + "]", "ddPerspiration[" + index + "]", "ddTremor[" + index + "]", 
    "ddAgitation[" + index + "]", "txtTemperature[" + index + "]", "txtPulse[" + index + "]", "txtRespirations[" + index + "]", 
    "txtBPSys[" + index + "]", "txtBPDias[" + index + "]", "txtMedication[" + index + "]", "txtAmount[" + index + "]");
    var isValid = true;
    ...
    if (page.dpDateLastUsed.Value() && page.autSign[index].Properties.IsReadOnly() && isValid) {
