/* Change Log - MR250 Progress Notes
 *
 *  18/05/2017  CP  App Created(Ticket #34222)
 *  06/06/2018 KS App updated(Ticket #37609)
 */
...
// page defaults
var PAGE_WIDTH = 820;
var PAGE_HEIGHT = 1175;

// Character Per line
var CHARS_PER_LINE = 65;
// Lines Per Page also used to register Repeater Controls
var LINES_PER_PAGE = 60; // 59
// Height range of notes for adding strikes
var NOTES_SPACE = 895;//886;
// Spaces for Images
var STICKER_LINES = 25;
var PHOTO_LINES = 26;
...
var stickerApps = Vitro.Elements.StickerNames;
// if user is in admin group
var isSuperUser = Vitro.Elements.GetUserGroup().IsSuperUser;
// if user is within group that can co-sign
var isCoSignUser = Vitro.Users().InGroup("Nurse") || Vitro.Users().InGroup("Senior Nurse") || Vitro.Users().InGroup("Doctor") || isSuperUser || Vitro.Users().InGroup("Supervision");
// Groups to be authorised for co-signs
var AUTHORISED_GROUPS = "Nurse^3|Senior Nurse^3|Doctor^3|Super User^3";
...
var USER_COLOURS = {
    "Pharmacy" : "128,196,216,153",
    "Super User" : "128,255,221,221",
    "Aurora Admin" : "128,220,220,220 ",
    "Senior Admin": "128,191,191,191",
    "Patient Revenue Team": "128,127,127,127",
    "Doctor": "128,191,213,239",
    "Allied Health Assist": "128,210,200,222",
    "Allied Health": "128,161,140,186",
    "Nurse" : "128,255,219,183",
    "Senior Nurse": "128,255,171,129"
};
...
// If this activity is SEALED or DELETED
    ...
    if (status == Vitro.STATUS.Sealed) {
        myApp.Show(Vitro.ACTIONS.Export);
        if(isSuperUser) {
...
function MyApp_Loaded() {
    ...
    if (status != Vitro.STATUS.Deleted) {
	    Vitro.Elements.SetAddressograph(myApp, pgNotes);
		...
		var key = Vitro.Elements.ObsChartToProgNotesKey;
...
function myApp_Actioned(action) {
    ...
    // get updated version of the status
    status = myApp.Activity.Properties.Status();
    if(status == Vitro.STATUS.Sealed)
    {
        myApp.Show(Vitro.ACTIONS.Export);
    }
    ...
    for (var i = 0; i < LINES_PER_PAGE; i++) {
        ...
        if (n[0] != "") {
            ...
            if (userEntryName != "" && userEntryName != "Completed") {                
                isStudentEntry = Vitro.Users(userEntryName).InGroup("Supervision");  
            }
        }
        ...
        if(n[1] !== undefined && (n[1] == SIG) && (name == CURRENT_USER || isSuperUser) && sigName != "" && pg.Control("lblUser[" + i + "]").Attribute(Vitro.LABEL.Content) != NOTE_COMPLETED){
            ...
            pos = 688 + "," + (228 + (rep_spacing * i));
            // make button at position
            CreateStrikeButton(pg, pos, i);
            // Making sure to remove duplication of strike in 2nd index when there is already 0 index on pgnotes for cosign 
        ...
        if(k[1] !== undefined && k[1] == "CoSign" && sigName != "" &&
            ...
            isCoSignUser && isStudentEntry && !pg.txtStrike[i].Properties.IsVisible()){
            // Positon the Dynamic Button
            pos = 160 + "," + (GetYPos(i) + 24);
...
function GetYPos(index) {
    // get y pos (textBox_ypos + (index * textBox_height) / lines_per_page)
    return Math.round(238 + ((index * 906) / LINES_PER_PAGE));
}
...
function ClickNoteEvent(pg, ctl, x, y) {
    if (y < NOTES_SPACE) {
        ...
        var theline = Math.floor(y * LINES_PER_PAGE / NOTES_SPACE) + 1;
        var index = theline - 1;
        ...
        if(index > 2){
            ...
            if(pg.lblUser[index-1].Attribute(Vitro.LABEL.Content).indexOf(IMAGE_ID) !== -1){
                StrikeSticker = true;
            }
        }
        ...
        if(Sig === NOTE_COMPLETED){
            ...
            if ((pg.lblUser[index].Attribute(Vitro.LABEL.Content).indexOf(IMAGE_ID) !== -1 || StrikeSticker) && (pg.lblUser[index].Attribute(Vitro.LABEL.Content).split("_")[0] == Vitro.Users().Property(Vitro.USER.Name) || isSuperUser)) {
                ...
                if (pg.txtStickerId.Value() !== "" || pg.txtStickerId1.Value() !== "") {
                    ...
                    if (pg.txtStrike[index].Properties.IsVisible()) {
                        if (!strikeList[theid]) {
...
function StrikePhotoAll(pg, index){
	...
	var SigName = pg.lblUser[Final_Index+1].Attribute(Vitro.LABEL.Content).replace(" Sig", "");
	var Sig = pg.lblUser[Final_Index+1].Attribute(Vitro.LABEL.Content).replace(SigName+" ", "");
	if(Sig === "Sig"){
		Final_Index += 2;
	}
...
function StrikeNoteAll(pg, start_index) {
	...
	for(var i = index; i > -1; i--) {
        ...
        if (pg.Properties.ID() === PageIDs[PageIDs.length > 1 ? 1 : 0].pageId && strikedFrom1stPage) {
            if (pg.Control("lblUser[" + i + "]").Attribute(Vitro.LABEL.Content).indexOf("Sig") !== -1) {
                strikedFrom1stPage = false;
                // commented break;, fix for STK strikes and unstrickes stops working after clicking STK multiple times. 
                // break;
            }
        }

		if(pg.Control("lblUser[" + i + "]").Attribute(Vitro.LABEL.Content) !== "" || i === index) {
			if(Mark){
				if(i === Final_Index) {
					Last = true;
					Mark = false;
	...
	if(!Found && pg.Control("lblUser[" + 0 + "]").Attribute(Vitro.LABEL.Content).match(/_Sticker/g) === null && pg.Control("lblUser[" + 0 + "]").Attribute(Vitro.LABEL.Content).match(/_Photo/g) === null) {
        ...
        for(var x = 0; x < PageIDs.length; x++) {
            ...
            if(pg.Properties.ID() === PageIDs[x].pageId && pg !== PageIDs[PageIDs.length - 1].pageId) {
...
function StickerStrike(pg, index) {
	...
	for(var i = index; i < LINES_PER_PAGE; i++){
		...
		if(Sig === "Sig"){
			...
			Final_Index = index_End;
            break;
		} else if (Sig === "Completed") {
            // Has been struck through and submitted, don't allow/attempt removal of strike.
            return;
        }
	}
	...
	if(MyStrikeButton.Attribute(Vitro.LABEL.Content) !== ""){
		...
		pg.txtStrikeName[Final_Index - 1].Value("");
	}
	...
	if(index < (LINES_PER_PAGE / 2)){
        ...
        for (var y = index_Start; y <Final_Index; y++) {
            ...
            // if the line is a sticker, add a strike
            SetStrike(pg, y);
        }
	}
	else{
        //find all lines that the sticker runs through
        for (var n = Final_Index-1; n >= index_Start+1; n--) {
			if(n !== Final_Index-1){
				Sticker_Strike = true;
			}
			else{
...
function SetStrike(pg, index) {
    ...
    if (pg.Control("txtStrike[" + index + "]").Properties.IsVisible() && strikeList[strikeId]) {
        // Hide, clear content and default the border colour
        pg.Control("txtStrike[" + index + "]").Hide().Value("");
		if(pg.Control("lblUser[" + index + "]").Attribute(Vitro.LABEL.Content).replace(CURRENT_USER + " ", "") !== SIG){
            pg.Control("txtStrikeName[" + index + "]").Hide().Value("");
		}
        ...
        if (strikeCount === 0) {
			if(Final_Index > 0){
				pg.Control("txtStrikeName[" + Final_Index-1 + "]").Hide().Value("");
			}
			EnableActions();
			myApp.Hide(Vitro.ACTIONS.Submit);
        }
    ...
    // if the label is hidden, we are toggling on
    else if (!pg.Control("txtStrike[" + index + "]").Properties.IsVisible()) {
        ...
        pg.Control("txtStrike[" + index + "]").Value(STRIKE_TXT).Show();
        if(!Sticker_Strike){
            pg.Control("txtStrikeName[" + index + "]").Value(init).Show();
		}
...
// add or remove strikes
function SetStrikeAll(pg, index) {
    
    // Make a key
    var strikeId = pg.Properties.ID() + "~" + index;
    // if the label is visible and the it is on the strike list, we are toggling off
    if (!StrikeAll && strikeList[strikeId] && (pg.Control("txtStrikeName[" + index + "]").Value() === "")){
        // Hide, clear content and default the border colour
        pg.Control("txtStrike[" + index + "]").Hide().Value("");
        pg.Control("txtStrikeName[" + index+ "]").Hide().Value("");
        // Remove it from the list
    ...
    // if the label is hidden, we are toggling on
        ...
        var init = Vitro.Elements.GetUserDetails().Initial +
            " " + (today.getDate() < 10 ? "0" : "") + today.getDate()+ "/" + ((today.getMonth() + 1) < 10 ? "0" : "") + (today.getMonth() + 1);
        // Show, set the content and clear border colour
        if(!pg.Control("txtStrike[" + index + "]").Properties.IsVisible()){
            // store strike in case of reload
            strikeList[strikeId] = true;
            strikeCount++;
            pg.Control("txtStrike[" + index + "]").Value(STRIKE_TXT).Show();           
            ...
            if (tempEvents.evSubmit === null) {
                tempEvents.evSubmit = myApp.Events.Actions(function(action) {
                    if (action == Vitro.ACTIONS.Submit) {
						...
						DONT_CLOSE = true;// not a constant. check use
                        return false;
                    }
                });
            }
...
function GetSignature(addSpace, cosgin) {
    var text = "";
    // get the signature (right aligned)
    var sig = "[" + GetUserDetails(addSpace ? cosgin : "").Normal + "]";
...
function AddSpace(spaceText, id) {
	...
	// CR:Change - update tab name only if clone page created + sort correctly
    var isCloned = false;
    ...
    // If were adding to a non-empty page, then make sure to leave a blank line
    if (oldLines > 0) oldLines++;
    // Set the user
    for (var i = oldLines; i < newLines + 2; i++) {
    ...
    // CR:Change- change page title to sys date if page cloned
    if(isCloned){
        pgNotes.Title(SetTitle());
    }
...
function SetNoteSidebar(startLine, endLine, pg, userName)
{
     // If not the first entry of the page then we need to account for the padded note
     var modifier = startLine > 0 ? 1 : 0;
     // Set the user k
     for (var i = startLine - modifier; i < endLine; i++) {
        pg.lblUser[i].Attribute(Vitro.LABEL.Content, userName, true);
        pg.txtDate[i].Attribute(Vitro.CONTROL.BackgroundColour, USER_COLOURS[Colour], true);
        pg.txtDate[i].Show();
     }
     // If not the first entry of the page, set BG colour for first row of the entry to transparent to accommodate for the additional padding
     if (modifier > 0 && startLine < endLine) pg.txtDate[startLine - modifier].Attribute(Vitro.CONTROL.BackgroundColour, COLOURS.TRANSPARENT, true);
}
...
function AddText(newNote, showDate) {
    ...
    var note = pgNotes.txtNotes.Value();
    var oldLines = CountLines(note, CHARS_PER_LINE);
    // Pad the oldlines so we have a space between entries
    if (oldLines > 0) oldLines++;
    // Ensure a min of two lines per entry
    // So if we are one line from the end of the page - insert a new line
    if (oldLines == (LINES_PER_PAGE - 1)) {
        note += "\r";
        oldLines++;
    }
    // If there is already content - pad with 2 lines.
    if (note !== "") { note += "\r\r"; }
    // Add new note
    note += newNote;
	...
	// CR: Ticket #50897
	// Add snap shot of text to summary app	if not empty
	if(newNote !== ""){
		var myNote = newNote.replace(/\n|\r/g, " ");
		myNote = ProcessNote(myNote, 88);
		var noteDocSummary = GetLines(myNote, 88, 0, 14);
		myApp.Activity.Extra(noteDocSummary);
	}
	// End Change
	...
	while (newLines > LINES_PER_PAGE) {
        ...
        if (cloned === false) {
            ...
            SetNoteSidebar(oldLines, LINES_PER_PAGE, pgNotesCloned, userName);
    ...
    if (newLines) {
        ...
        SetNoteSidebar(oldLines, newLines, pgNotes, userName);
...
function CloneNotePage(firstCloned) {
    // Clone the notes page
    var pgNotesCloned = myApp.PageManager.Clone(pgNotes);
    ...
    if (pgNotesCloned) {
        ...
        Vitro.Elements.GetClientLogo(myApp, pgNotesCloned.Control("imgClientLogo"));
		// set name, number and pantone
        // copy header
        pgNotesCloned.Deletable(false).Show(true).Order(2);
        ...
        if (firstCloned === false) {
            ...
            for (var i = 0; i < LINES_PER_PAGE; i++) {
                ...
                if (pgNotes.txtStrike[i].Value() !== "") {
                    pgNotesCloned.Control("txtStrike[" + i + "]").Value(pgNotes.txtStrike[i].Value()).Show();
                    pgNotesCloned.Control("txtStrike[" + i + "]").Attribute(Vitro.LABEL.TextColour, pgNotes.txtStrike[i].Attribute(Vitro.LABEL.TextColour), true);
					pgNotes.txtStrike[i].Value("").Hide();
                }

                if (pgNotes.txtStrikeName[i].Value() !== "") {
                    pgNotesCloned.Control("txtStrikeName[" + i + "]").Value(pgNotes.txtStrikeName[i].Value()).Show();
                    pgNotes.txtStrikeName[i].Value("").Hide();
                }
...
function RegisterNoteControls(pg) {
    ...
    pg.RegisterControls("txtPrefix", "txtSurname", "txtMRN", "dpDOB", "txtFirstName", "txtAddress1", "txtSuburb", "txtPostCode", "txtSex", "txtAdmittingDoctor", "txtHospitalLocation", "txtMobile", "pnlClickNote", "txtStickerNames");
    ...
    pg.RegisterRepeater("txtStrike", LINES_PER_PAGE);
    pg.RegisterRepeater("txtStrikeName", LINES_PER_PAGE);
...
function GetUserDetails(username) {
    ...
    return {
		Normal: Vitro.Elements.GetUserDetails(username).Details,
		PopUp: Vitro.Elements.GetUserDetails(username).Firstname + " " + Vitro.Elements.GetUserDetails(username).Lastname
	};
}
...
// function that sorts a list of groups alphabetically
function SortGroupListAlphabetically(Groups) {
...
function CreateStrikeButton(pg, pos, index) {
    ...
    // set properties
    strikeButton.Attribute(Vitro.CONTROL.Width, "80");
    ...
    if(pg.txtStrikeName[index].Value() === ""){
    ...
    strikeButton.Events.Click(function(pg, ctrl, xLocation, yLocation) {
        ...
        if(Finished){
            ...
            for (var i in coSignButtons) {
                var idx = (index - 2) < 1 ? 0 : (index - 2);
               
                if (coSignButtons[i].Index == idx) {
                    if (StrikeButton) { coSignButtons[i].auSign.Hide(); }
                    else { coSignButtons[i].auSign.Show(); }
                    break;
                }
            }
...
function CreateCoSignButton(pg, pos, index) {
    ...
    auCoSign.Attribute(Vitro.CONTROL.Height, "24");
  ...
  // Set Properties on different state
    ...
    auCoSign.Events.Change(function(pg, ctrl, oldValue, newValue) {
            ...
            if (newValue.SignStamp != entryName[0] && entryName[0] !== undefined) {
                ...
                pg.lblCoSign[index + 2].Attribute(Vitro.CONTROL.Height, 34, true);     
                auCoSign.Attribute(Vitro.AUTHORISATION.AuthoriseContent, "Co-Sign");    
...
function CreateDynamicNotePopup(pg) {
	...
	popupObj.lblUser.Value("Entry made by: " +  Vitro.Elements.GetUserDetails().Fullname).ReadOnly();

	popupObj.Dropdown = CreateDynamicDropdown(pg, "300", "10,435");
    popupObj.Dropdown.Attribute(Vitro.DROPDOWN.Add, "");
    // sort groups list alphabetically
    var sortedGroups = SortGroupListAlphabetically(Groups);
	...
	popupObj.txtNote.Events.Change(function(pg, ctrl, oldValue, newValue){
		if(newValue !== ""){
            ...
            popupObj.txtNote.Value(newValue);
			popupObj.btnOK.Enable();
		}
		else{
			popupObj.btnOK.Disable();
...
function CreateDynamicPhotoPopup(pg) {
    ...
    popupObj.fhDrawing.Attribute(Vitro.CONTROL.Width, "525");
    popupObj.fhDrawing.Attribute(Vitro.CONTROL.Height, "365");
    popupObj.fhDrawing.Attribute(Vitro.CONTROL.Position, "10, 40");
    ...
    popupObj.imgPhoto.Attribute(Vitro.CONTROL.Width, "525");
    popupObj.imgPhoto.Attribute(Vitro.CONTROL.Height, "365");
    popupObj.imgPhoto.Attribute(Vitro.CONTROL.Position, "10, 40");
	...
	popupObj.lblUser.Value("Photo taken by: " +  Vitro.Elements.GetUserDetails().Fullname).ReadOnly();
	...
	popupObj.imgPhoto.Events.Change(function(pg, ctrl, oldValue, newValue){
		if(newValue !== ""){
			popupObj.btnOK.Enable();
		}
		else{
...
function CreateDynamicStickerPopup(pg, addSticker) {
    ...
    if (addSticker) {
        ...
        // allowing for popup to get focus
        setTimeout(function() {
            popupObj.btnOpen.Focus();
        }, 100);
    }
    // Cancel button
    popupObj.btnCancel = CreateDynamicButton(pg, "150", "350, 80", "Cancel");
...
function StickerOkClickEvent(pg, ctrl, xlocation, ylocation) {
    ...
    // sort groups list alphabetically
    var sortedGroups = SortGroupListAlphabetically(Groups);
	xaimObj.UserColour = sortedGroups[0];
    // setup xaim
...
function ImageEnteredEvent() {
    ...
    // if image is set
    else if (popupObj.imgPhoto.Value().Base64 !== null) {
        popupObj.btnOK.Enable();
    }
}
